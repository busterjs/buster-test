...
function printUnsupported(reporter, unsupported) {
    ...
    for (var i = 0, l = unsupported.length; i < l; ++i) {
        str += "Skipping " + unsupported[i].context +
...
module.exports = {
    ...
    "context:start": function(context) {
        if (this.contexts.length == 0) {
            this.write('\n' + this.color.cyan(context.name) + "\n");
        } else {
            this.write('\n    ' + this.color.cyan(this.contexts[0]) +
                ' -> ' + this.color
                .cyan(context.name) +
                "\n\n");
    ...
    "context:unsupported": function(data) {
        printUnsupported(this, [{
            context: this.contexts.concat([data.context.name])
                .join(" "),
    ...
    "test:success": function(test) {
        this.write(this.color.green("       ✓ " + test.name) +
            "\n");
        this.printMessages();
    },
    ...
    "test:failure": function(test) {
        ...
        this.write(this.color[color]("       ✖ FAIL: " + test.name) +
            "\n");
        this.printMessages();
    ...
    "test:deferred": function(test) {
        this.write(this.color.cyan("        - " + this.getPrefix() +
            test.name) + "\n");
    },
    ...
    success: function(stats) {
        return stats.failures === 0 && stats.errors === 0 &&
            stats.timeouts === 0 && stats.tests > 0 && stats.assertions >
            0;
    },
    ...
    printExceptions: function(exceptions, label) {
        ...
        for (var i = 0, l = exceptions.length; i < l; ++i) {
            ...
            if (typeof this.printLog == "function" && fail.contextName &&
                fail.name) {
            ...
            if (fail.error) {
                ...
                if (fail.error.name && fail.error.name !=
                    "AssertionError") {
    ...
    printUncaughtExceptions: function() {
        this.printExceptions(this.uncaught, this.color.red(
            "Uncaught exception!"));
    },

((typeof define === "function" && define.amd && function (m) {
    define(["bane", "when", "lodash", "async", "platform"], m);
    ...
    function emit(runner, event, test, err, thisp) {
        var data = { name: test.name, environment: runner.environment };
    ...
    function testResult(runner, test, err) {
        if (!test) {
            err.environment = runner.environment;
    ...
    function parseEnvironment(env) {
        if (!env) { return null; }
        var environment = platform.parse(env);
    ...
    function TestRunner(opt) {
        ...
        this.environment = parseEnvironment(opt.environment);
    ...
    testRunner = TestRunner.prototype = bane.createEventEmitter({
        timeout: 250,
        onCreateListeners: [],
        ...
        runSuite: function (ctxs) {
            ...
            var d = when.defer();
            this.emit("suite:start", { environment: this.environment });
            if (this.environment) { emitConfiguration(this, ctxs); }
            if (this.focusMode) {
                this.emit("runner:focus", { environment: this.environment });
            }
        ...
        runContext: function (context) {
            ...
            asyncWhen(context).then(function (c) {
                ...
                var runTests = chainPromises(
                    _.bind(s, "runTests", fns, setUps(c), tearDowns(c), thisp),
                    end
                );

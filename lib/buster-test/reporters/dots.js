...
module.exports = _.extend(Object.create(consoleReporter), {
        ...
        for (var ctx in log) {
            for (var test in log[ctx]) {
                if (log[ctx][test].length) {
                    this.io.puts(this.color.green("Passed: " + ctx + " " + test));
                }
    ...
    printGlobalMessages: function () {
        ...
        this.io.puts(this.color.green("Global message log:"));
    ...
    printDeferred: function () {
        ...
        for (var i = 0, l = funcs.length; i < l; ++i) {
            this.io.puts(this.color.cyan("Deferred: " + funcs[i].context +
                                         " " + funcs[i].name));

            if (funcs[i].comment) {
                this.io.puts(this.color.grey(funcs[i].comment));
            }
        }

...
module.exports = {
    ...
    printExceptions: function (exceptions, label) {
        ...
        for (var i = 0, l = exceptions.length; i < l; ++i) {
            ...
            if (fail.error) {
                stack = stackFilter(fail.error.stack, this.cwd);
                ...
                if (fail.error.name && fail.error.name != "AssertionError") {
                    this.io.print(this.color.red(fail.error.name + ": "));
                }

                this.io.puts(this.color.red(fail.error.message));
    ...
    printUnsupported: function (unsupported) {
        ...
        this.io.print(this.color.yellow(str) + (!!str ? "\n" : ""));
    },
    ...
    printStats: function (stats) {
        ...
        if (stats.tests == 0) {
            this.io.puts(this.color.red("No tests"));
        } else {
            var color = this.success(stats) ? "green" : "red";
            this.io.puts(this.color[color](statStr.join(", ")));
            if (stats.assertions == 0) {
                this.io.puts(this.color.red("WARNING: No assertions"));
            }
        }

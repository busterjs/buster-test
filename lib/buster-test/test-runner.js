...
(function (B) {
    ...
    B.testRunner = B.util.extend(B.util.create(B.eventEmitter), {
        ...
        runTestFunction: function (test, testCase) {
            var result = asyncFunction(test.func, testCase), self = this;
        ...
        runSetUps: function (setUps, test, testCase) {
            ...
            return noisyAll.call(this, "runSetUp", setUps, testCase);
        },

        runSetUp: function (setUp, testCase) {
            return asyncFunction(setUp, testCase);
        },

        runTearDowns: function (tearDowns, test, testCase) {
            this.emit("test:tearDown", { name: test.name });
            return noisyAll.call(this, "runTearDown", tearDowns, testCase);
        },

        runTearDown: function (tearDown, testCase) {
            return asyncFunction(tearDown, testCase);
        },

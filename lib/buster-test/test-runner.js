    ...
    B.testRunner = B.util.extend(B.util.create(B.eventEmitter),{
        ...
        runTest: function (test, testCase, setUps, tearDowns) {
            ...
            B.util.nextTick(function () {
                ...
                try {
                    self.runSetUps(setUps, testCase, test);
                ...
                if (testResult && testResult.then) {
                    self.emit("test:async", { name: test.name });
                }

                var err2 = self.runTearDowns(tearDowns, testCase, test);
                err = err2 || err;
        ...
        runSetUps: function (setUps, testCase, test) {
        ...
        runTearDowns: function (tearDowns, testCase, test) {

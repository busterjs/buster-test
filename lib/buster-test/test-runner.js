    ...
    buster.testRunner = buster.util.extend(buster.util.create(buster.eventEmitter), {
        ...
        run: function (context) {
            ...
            for (var i = 0, l = tests.length; i < l; ++i) {
                ...
                if (!error) {
                    try {
                        var expectedCount = context.testCase.expectedAssertions;

                        if (typeof expectedCount == "number" &&
                            this.assertionCount() != expectedCount) {
                            var assertCountError = new Error("Expected " + expectedCount + " assertions, ran " + this.assertionCount());

...
(function (B) {
    ...
    B.testRunner = B.extend(B.create(B.eventEmitter), {
        ...
        run: function (context) {
            ...
            sequential([
                B.bind(this, all, "runTest", randomize(context.tests),
                       setUps, tearDowns, testCase),
                B.bind(this, all, "run", randomize(context.contexts))
            ]).then(function () {
        ...
        runTest: function (test, setUps, tearDowns, testCase) {
            ...
            function cleanUp(err) {
                self.testSetupError = err;

                function done(err2) {
                    delete self.testSetupError;

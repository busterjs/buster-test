...
(function () {
        ...
        function runOne(item) {
            ...
            try {
                if (typeof opt.method == "function") {
                    resolution = opt.method.apply(opt.ctx, [item].concat(args));
                } else {
                    resolution = item.apply(opt.ctx, args);
                }
            } catch (e) {
                promise.reject(e);
                return;
    ...
    buster.promise.parallelize = function (items, opt) {
        ...
        for (var i = 0, l = items.length; i < l; ++i) {
            if (method) {
                promise = method.apply(opt.ctx, [item].concat(args));
            } else {
                promise = item.apply(opt.ctx, args);
            }

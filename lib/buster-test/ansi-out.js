        ...
        var list = {
            refitLabels: function (label) {
                ...
                for (var i = 0, l = labels.length; i < l; ++i) {
                    if (labels[i].length < width) {
                        printAt({ x: 0, y: labels[i] }, formatLabel(labels[i]));
                    }
                }
            },
            ...
            add: function (label) {
                ...
                labels.push(label);
                lines[label] = { x: width + 2, y: current };
            ...
            print: function (label, text) {
                io.print(buster.ansiOut.move(function () {
                    var str = this.up(current - lines[label].y) +
                        this.fwd(lines[label].x) +
                        text;
                    lines[label].x = buster.ansiOut.stripSeq(text).length;
